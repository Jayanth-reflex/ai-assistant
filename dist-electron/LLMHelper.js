...
Estimated Time: [X minutes]`
    ...
    async extractProblemFromImages(imagePaths) {
        const imageParts = await Promise.all(imagePaths.map(this.fileToGenerativePart.bind(this)));
        ...
        const basicResult = await this.model.generateContent([basicPrompt, ...imageParts]);
    ...
    async debugSolutionWithImages(problemInfo, currentCode, debugImagePaths) {
        const imageParts = await Promise.all(debugImagePaths.map(this.fileToGenerativePart.bind(this)));
